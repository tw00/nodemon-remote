...
const { handleRequest } = require("./remote");
const config = require("./config");

module.exports = function createRemoteServer(context) {
  const server = http.createServer(
    makeRequestListener({
      handleWebhookRequest: (req) => handleWebhookRequest(req, context),
      handleRequest: (req) => handleRequest(req, context),
    })
  );
